...
def self_consistency(examples: List[Dict], anchor_text: List[str] = []):
    # 提取所有预测
    predictions = [example['output'] for example in examples]
    ...
    # 使用最常见答案更新原始 example 数据
    result = examples[0].copy()  # 基于第一个 example
    result['output'] = most_common_answer
    result['consistency_scores'] = dict(answer_counts)
    return result
