...
from .templates import CtxPrompt, ApiReturn, RetrievalInstruction, ChatHistory, ChatMessage
from .datasets import StrategyQA, WikiMultiHopQA, WikiAsp, ASQA
from .utils import Utils, NoKeyAvailable, openai_api_call
from .generator import rag_drafting_generator, RagDraftingResponse, rag_verifier_generator, direct_generator
import asyncio
import time
...
class QueryAgent:
    def __init__(
        ...
        # generation args
        self.final_stop_sym = retrieval_kwargs.get('final_stop_sym', '<finish>')
        ...
        self.ctx_increase = retrieval_kwargs.get('ctx_increase', 'append')
    ...
    def retrieve(
        ...
        if len(queries) and type(queries[0]) is list:  # nested queries
            ...
            subsets = self.retriever.k_means_retrieve(
                queries=queries,
                topk=self.ret_topk,
                max_query_length=mql
            )
    ...
    def complete(
        ...
        if is_chat_model:
            messages = [  # system
                ...
                # {'role': 'system', 'content': f'If you believe the response is complete, end it with {self.final_stop_sym}.'},
            ] + [  # current example
    ...
    def prompt(
        ...
        if speculative:
            return asyncio.run(self.speculative_ret_prompt(queries, api_key=api_key))
    ...
    def ret_prompt(
        ...
        queries: List[CtxPrompt],
        api_key: str = None,
        max_iteration: int = 5,  # TODO: too high?
    ):
        ...
        while len(queries) and max_gen_len < self.max_generation_len and step_ind <= max_iteration:
            ...
            if self.look_ahead_steps:  # generate a fixed number tokens for retrieval
                ...
                if self.look_ahead_truncate_at_boundary:
                    print('doing truncating at boundary')
                    apireturns = [ar.truncate_at_boundary(self.look_ahead_truncate_at_boundary) for ar in apireturns]
                
                # 输出ar看tokens和text
                # for ar, (_, q) in zip(apireturns, queries):
                #     print('ar.tokens: ', ar.tokens)
                #     print('ar.text: ', ar.text)
                #     print('-'*30+'\n')
            ...
            if self.use_gold:
                ...
                # 注意这里是non_empty_query才进行下一步issue，否则直接接受
            ...
            if self.ret_frequency:
                ...
                if self.truncate_at_prob > 0:
                    ...
                    max_gen_len += int(np.min([ar.num_tokens for ar in apireturns]))
                    generate_queries = [ar.text for ar in apireturns]  # always use newly generated as query
                    print("ar after complete :", apireturns[0].text)
                else:
    ...
    async def speculative_ret_prompt(
        self,
        queries: List[CtxPrompt],
        api_key: str = None,
        max_iteration: int = 5,
        m_drafter: str = "gpt-4o-mini",
        m_verifier: str = "gpt-4o",
        low_p : float = 0.8,
    ):
        query = queries[0].format(use_ctx=self.use_ctx, is_chat_model=True, api_key=api_key)[0] # only support single query
        ...
        MAX_ROUND = 5
        round = 0
        chat_history = ChatHistory(5)
        # print(f"query = {query}")
        while round < MAX_ROUND:
            # query_message = ChatMessage(role="user", content=query)
            # Draft Model doing Directly answer without RAG
            direct_generation : ApiReturn = await direct_generator(
                model_name=self.model,
                instruction=query,
                chat_history=chat_history,
            )
            if direct_generation.text == "<finish>":
                print("Answer is finished")
                break
            print(f"Before truncate direct_generation text : {direct_generation.text}")
            direct_generation = direct_generation.truncate_at_boundary("sentence")
            print(f"direct_generation text : {direct_generation.text}")
            check_prob = direct_generation.check_prob(low_p)
            print(f"check_prob : {check_prob}")
            if check_prob:
                final_outputs[0] += direct_generation.text
                final_probs[0].extend(direct_generation.token_probs)
                chat_history.add_message(
                    ChatMessage(role="assistant", content=direct_generation.text)
                )
            else:
                retrieve_query = await direct_generation.use_as_query(mask_prob=0.8, mask_method='wholeterm-askquestion')
                print(f"retrieve_query = {retrieve_query[0]}")
                sampled_docs = self.retriever.k_means_retrieve([retrieve_query[0]])
                # RAG Drafting
                rag_drafts: list[tuple[RagDraftingResponse, float, ApiReturn]] = await asyncio.gather(
                    *[
                ...
                rag_verifications: list[tuple[str, float]] = await asyncio.gather(
                    *[
                        ...
                        for subset, (rag_drafting_response, _, _) in zip(sampled_docs, rag_drafts)
                    ]
                )
                
                if self.debug:
                    for i in range(len(rag_drafts)):
                        print(f"RAG drafting response: {rag_drafts[i][0].response}")
                        print(f"RAG drafting rationale: {rag_drafts[i][0].rationale}")
                        print(f"RAG drafting prob: {rag_drafts[i][1]}")
                        print(f"RAG verification: {rag_verifications[i][0]}")
                        print(f"RAG verification prob: {rag_verifications[i][1]}")
                    
                max_score = -1.0
                best_answer = -1

                for i in range(len(rag_drafts)):
                    p_draft = rag_drafts[i][1]
                    p_self = rag_verifications[i][1]
                    score = p_draft * p_self
                    # print(f"Index: {i}, Score: {score}")  # 可选：打印每个组合的得分
                    if score > max_score:
                        max_score = score
                        best_answer = i
                        
                best_draft = rag_drafts[best_answer]
                # print(f"\nQuestion:\n ------ \n{query}\n\n")
                print(f"Response:\n ------ \n{best_draft[0].response}")
                print(f"Rationale: \n ------ \n{best_draft[0].rationale}\n")
                
                final_outputs[0] += best_draft[0].response
                final_probs[0].extend(best_draft[2].token_probs)
                final_retrievals[0].append(best_draft[0].rationale)  
                chat_history.add_message(
                    ChatMessage(role="documents", content=rag_drafts[best_answer][0].rationale)
                )
                chat_history.add_message(
                    ChatMessage(role="assistant", content=rag_drafts[best_answer][0].response)
                )
...
if __name__ == '__main__':
    ...
    # 定义查询
    # test for retriever
    # queries = ["Which film has the director who died first, Promised Heaven or Fire Over England?"]

    # # 调用新增的 k_means_retrieve 方法
    # subsets = retriever.k_means_retrieve(
    #     queries=queries,
    #     topk=5,  # 每个查询检索5个文档
    #     k=3,     # 聚类为3类
    #     n=2,     # 实际检索10个文档（5 * 2）
    ...
    # print(subsets)
    # print(f'subsets len : {len(subsets)}')
    # print(f'element len : {len(subsets[0])}')
    
    # exit()
    
    retrieval_kwargs['retriever'] = retriever
    retrieval_kwargs['debug'] = args.debug
    retrieval_kwargs['final_stop_sym'] = args.final_stop_sym or ('!@#$%^&*()' if Utils.no_stop(model=args.model) else '\n\n')
    ...
    if not args.multiprocess:  # query for one process
        ...
        with tqdm(total=len(data)) as pbar, open(args.output, 'w') as fout:
            for b in range(0, len(data), args.batch_size):
                ...
                prompts = [CtxPrompt.from_dict(example) for example in batch]
                generations, probs, retrievals, traces = qagent.prompt(prompts, api_key=key, speculative=args.speculative)
                retrievals = retrievals or [None] * len(generations)
                traces = traces or [None] * len(generations)
                for example, generation, prob, retrieval, trace in zip(batch, generations, probs, retrievals, traces):
                    example['output'] = generation
