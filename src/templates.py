...
class CtxPrompt:
    ...
    @classmethod
    async def chatgpt_get_response(cls, prompt: Union[str, List[str]], examplars: List[List[Tuple[str, str]]] = [[]], max_tokens: int = 2048, api_key: str = None):
        ...
        for p, es in zip(prompt, examplars):
            messages = [
                {'role': 'user' if i == 0 else 'assistant', 'content': e[i]} 
                for e in es for i in range(2)
            ] + [
                {'role': 'user', 'content': p},
            ]
            try:
    ...
    @classmethod
    async def extract_low_confidence_terms_rule(
        cls,
        tokens: List[str],
        probs: List[float],
        low: float = 0.0,
        ...
        if len(spans) <= 0:
            return terms
        if expand:
            new_terms = await cls.extract_constituents(tokens, spans=spans, api_key=api_key)
    ...
    @classmethod
    async def extract_constituents(cls, tokens: List[str], spans: List[Tuple[int, int]], api_key: str = None, special_symbol: str = '*', debug: bool = False):
        examplars = [
        ...
        responses = await cls.chatgpt_get_response(prompt=prompts, examplars=[examplars] * len(prompts), api_key=api_key)
    ...
    async def ask_question_text(
        ...
        responses = await cls.chatgpt_get_response(cases, api_key=api_key)
    ...
    @classmethod
    async def get_queries_from_text_for_retrieval(
        cls,
        context: str,
        tokens: List[str],
        probs: List[float],
        ...
        if detect_low_terms:
            terms = await cls.extract_low_confidence_terms_rule(tokens=tokens, probs=probs, low=low, api_key=api_key)
        if debug:
            print('2->', terms)
        if decontextualize:
            ...
            questions = await cls.ask_question_text(context=context, text=text, terms=terms, api_key=api_key)
        if detect_low_terms:
            if decontextualize:
                for term in terms:
...
class ApiReturn:
    ...
    def truncate_at_boundary(self, unit: str = 'sentence'):
        ...
        if unit == 'sentence':
            ...
            print('len(self.text) = ', len(self.text))
            if break_at > 0 and break_at < len(self.text):  # truncation
                if self.has_tokens:
                    # print("assert self.has_tokens = true")
                    i = 0
                    for i in range(self.num_tokens):
                        if self.offsets[i] >= break_at:
                            break_at = self.offsets[i]
                            break
                    assert i > 0
                    self.tokens = self.tokens[:i]
                    self.probs = self.probs[:i]
                ...
                # print("text after truncation : ", self.text)
                self.finish_reason = 'boundary'
        else:
            raise NotImplementedError
        return self
    ...
    async def use_as_query(
        ...
        if mask_prob:
            if mask_method == 'simple':
                keep = [(t.token if p > mask_prob else ' ') for t, p in zip(self.tokens, self.probs)]
                ...
                text_tokens = [t.token for t in self.tokens]
                keep = await CtxPrompt.get_queries_from_text_for_retrieval(
                    context=context,
                    tokens=text_tokens,
                    probs=self.probs,
                    low=mask_prob,
    ...
    def check_prob(
        ...
        for i, p in enumerate(self.probs):
            # print(f'token = {self.tokens[i]}, p = {p}, low_prob = {low_prob}')
            if p <= low_prob:
                return False
        return True
    ...
    def get_text(self):
        ...
        for message in self.messages:
            ...
            if role == 'user':
                ...
                document_messages.append(f"Assistant: {content}\n")
            elif role == 'assistant':
        ...
        # 按顺序组合所有消息
        combined_text = ''.join(user_messages + document_messages + assistant_messages)
